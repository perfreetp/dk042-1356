...
const RecordsPage: React.FC = () => {
  ...
  const summary = useMemo(() => {
    ...
    if (total === 0) {
      return { total, passed: 0, avgScore: 0, avgDuration: 0, errorRate: 0 }
    }
    ...
    const totalErrors = records.reduce(
      (s, r) => s + r.borrowErrors.length + r.returnErrors.length,
      0
    )
    const maxPossibleErrors = records.length * 8
    const errorRate = Math.round((totalErrors / maxPossibleErrors) * 100)
    return { total, passed, avgScore, avgDuration, errorRate }
  }, [records])

  const handleViewDetail = (record: PracticeResult) => {
    console.log('[Records] 查看成绩详情:', record.id)
    Taro.navigateTo({
  ...
  return (
    <ScrollView scrollY className={styles.page}>
      <View className={styles.header}>
        ...
        <Text className={styles.pageSub}>回顾练习记录，针对薄弱点强化训练</Text>
            ...
            >
              ...
              <View className={styles.recordAction}>
                <View className={styles.actionBtn}>
                  <Text className={styles.actionBtnText}>查看错误分析 →</Text>
                </View>
              </View>
            </View>
          ))
